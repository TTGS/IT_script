-- drop FUNCTION check_datatype( v_data text, v_datatype varchar);

select check_datatype('a','bigint');
select check_datatype('132','bigint');

CREATE or replace FUNCTION check_datatype( v_data text, v_datatype varchar)    
RETURNS boolean    AS $$
DECLARE
-- 我自己的参数
p_data text:=v_data;
p_datatype varchar:=lower(v_datatype);
p_result boolean  :=null;
-- 系统异常捕获
...
begin
case 
	when p_datatype in ( 'point' ) 
	 then select  p_data ~ '^\d[,]\d$' into p_result ; 
	when p_datatype in ( 'varchar','text','char') 
	 then select p_data ~ '*' into p_result ; 
	when p_datatype in ( 'smallint','int','bigint','double precision' ,'real','decimal','numeric','decimal') 
	 then  select   p_data ~ '^(\d|)*(\.|)(|\d)*(|\d)$' into p_result ;   
	when p_datatype in ( 'boolean' ) 
	 then  select   lower(p_data) in (TRUE,'t','true','y','yes','on','1',FALSE,'f','false','n','no','off','0') into p_result ; 
	else null ;
end  case ;
