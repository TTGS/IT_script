...
v_city   character varying DEFAULT 'F'::bpchar, 
v_street character varying DEFAULT 'F'::bpchar, 
v_date   character varying DEFAULT 'B'::bpchar, 
...
case 
	when  upper(p_thr) in ('R' ,'','TRUE','T','FALSE','F')   or p_thr is null   then 
	    -- 内容结果是随机内容
...
case 
   ...
   when  length(p_check)=1 and p_check ~* '\d{1}' then 
   		-- 检查长度为1，成功就赋值 
   		expect_result:='v_check  is set  True ,and the  program''s result is not 17-digit. We only have 17-digit calculation methods .' ;
		p_temp_check:=p_check;
   else  
        -- 如果以上内容检查都失败了，那么这就是我不知道的内容了，会抛出异常。
        -- 这里判断对传进来的参数进行数字检查， 
        -- 如果检查成功，那么就直接扔出异常结束。
       expect_result:='v_check have to F(FALSE,NO,N) ,  T(TRUE,YES ,Y) or  1-digit numeric ,but ('||p_check||') is not anyone.'  ;
       RAISE exception '';
end case  ;
