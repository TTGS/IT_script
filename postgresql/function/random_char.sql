-- drop FUNCTION random_char(v_len bigint,v_case text  )
/*
 * select random_char(5,'n');
 * select random_char(5,'y');
 * select random_char(5,null);
 * select random_char(null);
*/
CREATE or replace FUNCTION random_char(v_len bigint,v_case text default 'N')    
RETURNS text    AS $$
DECLARE
   -- 我自己的参数
   p_len       bigint:=v_len;
